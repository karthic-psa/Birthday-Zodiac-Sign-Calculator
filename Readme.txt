Design comparison:
1.	Both the designs are similar but for a few changes. In both the designs we see that we have a main method to input the date of birth and calls the other 3 methods to check whether the date entered is valid (checkDate method), get the zodiac sign (zodiacSign method) and the method to find the day of the week (DayofWeek method). 
